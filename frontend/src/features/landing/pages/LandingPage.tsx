...
const features = [
    {
        ...
        description: "Send and receive money instantly with our smart contact lookup — no manual IDs, just names.",
    ...
    },
    {
        ...
        description: "Divide expenses effortlessly among friends. Send split requests and settle shared costs directly from the app.",
    ...
    },
    {
        ...
        description: "Understand your spending with real-time Recharts visualisations, cash-flow history, and category breakdowns.",
    ...
    },
    {
        ...
        description: "JWT auth, bcrypt hashing, Helmet middleware, and strict CORS policies keep every transaction safe.",
...
/** Adds "in-view" class to elements with class "lp-fadeup" when they enter the viewport */
function useFadeup() {
    useEffect(() => {
...
/** Drives the scroll progress bar */
...
export function LandingPage() {
    ...
    return (
        <div className="min-h-screen bg-[#f4f7f8] text-[#172026]">

            {/* ── Scroll Progress Bar ──────────────────────────────────────── */}
            <div ref={progressRef} className="scroll-progress" aria-hidden="true" />

            {/* ── Navigation Bar ───────────────────────────────────────────── */}
            <header className="fixed top-0 left-0 right-0 md:top-4 md:left-1/2 md:-translate-x-1/2 z-50 w-full md:w-[calc(100%-2.5rem)] max-w-215 bg-white/95 backdrop-blur-md border-b border-[#e6eaee] md:border md:rounded-full shadow-sm md:shadow-[0_4px_24px_rgba(20,36,45,0.10)]">
            ...
            <main>
                ...
                <section className="relative overflow-hidden pt-24">
                    ...
                    <div className="relative max-w-6xl mx-auto px-6 pt-12 pb-20 grid lg:grid-cols-2 gap-16 items-center">
                        {/* Left: Copy */}
                        <div>
                            <span className="lp-fadeup inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase bg-[#0d6b5f]/10 text-[#0d6b5f] mb-6" style={{ "--fd": "0s" } as React.CSSProperties}>
                                Open-Source FinTech Project
                            </span>
                            <h1 className="lp-fadeup text-4xl sm:text-5xl font-extrabold text-[#0f1419] leading-[1.1] tracking-tight mb-5" style={{ "--fd": "0.08s" } as React.CSSProperties}>
                                Seamless P2P Payments &{" "}
                                <span className="text-[#0d6b5f]">Smart Wallet</span>{" "}
                                Management.
                            </h1>
                            <p className="lp-fadeup text-lg text-[#4d5c65] leading-relaxed mb-8 max-w-xl" style={{ "--fd": "0.15s" } as React.CSSProperties}>
                                A secure platform to track transactions, split bills, and
                                visualise your financial data in real-time — built with a
                                modern full-stack architecture.
                            </p>
                            <div className="lp-fadeup flex flex-wrap items-center gap-4" style={{ "--fd": "0.25s" } as React.CSSProperties}>
                                {user ? (
                                    <Link to="/dashboard" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-base font-bold text-white bg-[#0d6b5f] hover:bg-[#094d45] transition-colors no-underline shadow-sm">
                                        Launch Wallet <ArrowRight size={16} />
                                    </Link>
                                ) : (
                                    <Link to="/signup" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-base font-bold text-white bg-[#0d6b5f] hover:bg-[#094d45] transition-colors no-underline shadow-sm">
                        ...
                        {/* Right: App Mockup */}
                        <div className="lp-fadeup relative flex items-center justify-center" style={{ "--fd": "0.2s" } as React.CSSProperties}>
                            <div className="w-full max-w-md bg-white rounded-[28px] border border-[#dce4e8] shadow-2xl shadow-[#0d6b5f]/5 overflow-hidden">
                                ...
                                <div className="p-5 space-y-4">
                                    <div className="bg-[#0d6b5f] rounded-2xl p-5 text-white">
                                        <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-1">Current Balance</p>
                                        <p className="text-3xl font-extrabold tracking-tight">₹ 24,830.00</p>
                                        <div className="flex gap-3 mt-4">
                                            <span className="text-xs bg-white/20 rounded-full px-3 py-1 font-semibold">+ Add Money</span>
                                            <span className="text-xs bg-white/20 rounded-full px-3 py-1 font-semibold">↑ Send</span>
                                        </div>
                                    </div>

                                    <div className="bg-[#f4f7f8] rounded-2xl p-4 border border-[#e6eaee]">
                                        <div className="flex justify-between items-center mb-3">
                                            <p className="text-xs font-bold text-[#0f1419]">Cash Flow — Last 6 Months</p>
                                        ...
                                        <div className="flex items-end gap-2 h-16">
                                            {[55, 80, 40, 90, 65, 75].map((h, i) => (
                                                <div
                                                    ...
                                                    style={{
                                                        height: `${h}%`,
                                                        backgroundColor: i % 2 === 0 ? "#0d6b5f" : "#94a3b8",
                                                        opacity: 0.7 + i * 0.05,
                                                    }}
                                                />
                                            ))}
                                        ...
                                        <div className="flex justify-between mt-2">
                                            {["Jan", "Feb", "Mar", "Apr", "May", "Jun"].map((m) => (
                                                <span key={m} className="text-[10px] text-[#9aa3ad]">{m}</span>
                                            ))}
                                        </div>
                                    </div>

                                    <div>
                                        <p className="text-xs font-bold text-[#0f1419] mb-2.5">Recent Transactions</p>
                                        {[
                                            ...
                                            <div key={tx.label} className="flex items-center justify-between py-2 border-b border-[#edf1f3] last:border-b-0">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <span className="text-xs font-semibold text-[#1e293b]">{tx.label}</span>
                                                </div>
                            ...
                            <div className="absolute -bottom-4 -left-4 bg-white rounded-2xl border border-[#dce4e8] shadow-lg px-4 py-2.5 flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-[#0f1419]">Real-time sync</span>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section className="max-w-6xl mx-auto px-6 py-16">
                    <div className="text-center mb-14">
                        <p className="lp-fadeup text-xs font-bold uppercase tracking-widest text-[#0d6b5f] mb-3">
                            What's Inside
                        </p>
                        <h2 className="lp-fadeup text-3xl font-extrabold text-[#0f1419] tracking-tight" style={{ "--fd": "0.08s" } as React.CSSProperties}>
                            Everything you need in one place
                        </h2>
                        <p className="lp-fadeup mt-3 text-[#4d5c65] max-w-lg mx-auto" style={{ "--fd": "0.15s" } as React.CSSProperties}>
                            A full set of financial tools, built from the ground up with developer-grade quality.
                        </p>
                    </div>

                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        {features.map(({ icon: Icon, title, description, accent, border }, i) => (
                            <div
                                key={title}
                                className={`lp-fadeup bg-white rounded-[20px] border ${border} p-6 flex flex-col gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200`}
                                style={{ "--fd": `${i * 0.08}s` } as React.CSSProperties}
                            >
                                <div className={`w-10 h-10 rounded-xl ${accent} flex items-center justify-center shrink-0`}>
                                    <Icon size={20} />
                                </div>
                                <div>
                                    <h3 className="text-sm font-extrabold text-[#0f1419] mb-1.5">{title}</h3>
                                    <p className="text-sm text-[#4d5c65] leading-relaxed">{description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <section className="border-t border-[#dce4e8] bg-white py-14">
                    <p className="lp-fadeup text-center text-xs font-bold uppercase tracking-widest text-[#9aa3ad] mb-8">
                        Powered By
                    </p>
                    <div className="lp-marquee-wrap">
                        <div className="lp-marquee-track gap-3 px-4">
                            {/* Duplicate for seamless loop */}
                            {[...stack, ...stack].map(({ label, color }, i) => (
                                <span
                                    key={`${label}-${i}`}
                ...
                {/* ── CTA Banner ───────────────────────────────────────────── */}
                <section className="max-w-6xl mx-auto px-6 py-16">
                    <div className="lp-fadeup bg-[#0d6b5f] rounded-[28px] px-8 py-14 text-center text-white relative overflow-hidden">
                        ...
                        <p className="relative text-[#a7d4cf] max-w-sm mx-auto mb-8 text-sm leading-relaxed">
                            Create a free account, add your wallet balance, and explore the full feature set in minutes.
                        </p>
                        <Link
