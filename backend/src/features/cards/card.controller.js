...
export const setPrimaryCardController = async (req, res) => {
    try {
        const card = await setPrimaryCardForUser(req.user.id, req.params.cardId);

        return res.status(200).json({ message: "Primary card updated successfully", card });
    } catch (err) {
